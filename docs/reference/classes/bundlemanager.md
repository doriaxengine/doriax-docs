...
`enemies/EnemyShip`). At runtime you spawn an instance by that name (or ID) with
...
This call is **generated by the export step** for every `.bundle` file in the project, so
you normally never write it yourself. It is C++-only and not exposed to Lua.
