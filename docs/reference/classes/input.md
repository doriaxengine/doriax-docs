...
This value can also be set manually with [Engine.setMousePosition](engine.md#setmouseposition). When [Engine.mouseMode](engine.md#mousemode_1) is `CAPTURED`, the position is a virtual canvas-space position accumulated from relative mouse movement and may move outside the normal canvas range.
