...
Open **Edit → [Editor Settings](../editor/editor-settings.md#desktop) → Desktop** and use
the **Compiler** dropdown. The editor detects available compilers (GCC, Clang,
MSVC/Visual Studio) and pairs each one with a compatible CMake generator:
...
Your selection is stored **per project**, so a project that needs MSVC and one that
needs MinGW can coexist, and it is **remembered across sessions** — a project that has no
kit yet inherits the compiler you last chose, so you only need to set it once (rather
...
!!! danger "Your compiler must match the editor's own C++ ABI"
    ...
    compilers** in the **Compiler** dropdown — hover one to read why. If the kit a
    project already had is no longer detected, the dropdown falls back to **Default**
    and warns that builds still use the stored one until you press OK. This is a
...
Open **File → Export Project... → Desktop** and use **Build Jobs** to tune this. The
field starts at the automatic value — one job per logical CPU thread — and the same
number is used for Play-mode script builds, not just for the export you are configuring.

| Value | Effect |
| --- | --- |
| Automatic (default) | One job per logical CPU thread of the machine running the build |
...
Projects saved by older editors carry the setting as `cmakeBuildJobs` in `project.yaml`.
Opening one migrates the value into the editor settings once — see [Editor
Settings](../editor/editor-settings.md#migrating-from-older-projects).
...
!!! note "MSVC rebuilds scripts when the value changes"
    ...
    Since that changes compile flags, changing **Build Jobs** triggers a full script
...
**Project → [Project Settings](../editor/project-settings.md#directories) → Directories →
Script Directories** changes that. Add any
...
| `The C compiler ... is not able to compile a simple test program` together with `CMAKE_LINKER-NOTFOUND` or `TRK0005` | A custom compiler (usually Clang) was configured with the Visual Studio generator. Install Ninja and re-select the Clang compiler in Editor Settings, or switch to the MSVC compiler. Editor versions before June 2026 allowed this broken combination. |
...
| `This Doriax editor requires MSVC-compatible C++ plugins` (or `... MinGW/GNU C++ plugins`) at the **configure** step | The selected compiler's ABI does not match the editor's, and the generated CMake project stops before compiling anything. Pick a compatible compiler in **Editor Settings → Desktop → Compiler**; this mainly appears with a compiler saved before the ABI check existed, or one selected outside the editor. See [Choosing a compiler](#choosing-a-compiler). |
| The build uses `C:\msys64\ucrt64\bin\g++.exe` even though Visual Studio is installed and **Compiler** is **Default** | An MSYS2 `cmake` came earlier on `PATH` than the Visual Studio one, and older editors let CMake choose the toolchain — which that CMake resolves to Ninja with MinGW GCC. Update the editor (Default now pins an ABI-compatible toolchain), or select **MSVC** explicitly in **Editor Settings → Desktop → Compiler**. |
...
| `No CMAKE_C_COMPILER could be found` (with `Building for: Visual Studio 17 2022`) | CMake's Visual Studio auto-detection found no usable compiler. Current editors no longer route **Default** through that auto-detection on Windows, so update the editor first. Otherwise pick an explicit compiler in **Editor Settings → Desktop → Compiler** (MSVC, or Clang). The choice is remembered across projects, so you only set it once. If even MSVC fails here, your Visual Studio C++ toolset is incomplete — repair "Desktop development with C++" in the Visual Studio Installer. |
| `internal compiler error: Killed` (GCC/Clang) or `fatal error C1060: compiler is out of heap space` (MSVC) | The parallel build ran out of memory. Lower **Export Project → Desktop → Build Jobs** (try `1` or `2`) — see [Parallel builds](#parallel-builds). |
...
- [Editor Settings](../editor/editor-settings.md) — where the compiler and CMake path are configured
